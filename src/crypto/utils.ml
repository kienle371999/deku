...
let calculate_bit_length number =
  let rec int_to_bit acc number =
    if number = 0 then
      ...
      int_to_bit ((number land 1) :: acc) (number lsr 1) in
...
let calculate_combination k n p =
  let combination =
    Int.to_float (factorial n)
    /. (Int.to_float (factorial k) *. Int.to_float (factorial (n - k))) in
...
let rec calculate_binomial_distribution k w p =
  ...
  | _ ->
    calculate_combination k w p +. calculate_binomial_distribution (k - 1) w p
